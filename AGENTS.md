...
6. **엔진 표면 문서에 자동 반영 경로는 없다** — `references/image/{grok-imagine,seedream-character-reference-sheets,midjourney-feed-diagnosis}.md`는 2026-07-25에 해체된 위성 스킬(`image-prompt-engine-adapters`)에서 이관됐다. 그 스킬은 에이전트가 직접 쓸 수 있는 델타 레인이었고, 이 트리는 아니다. 백그라운드 큐레이터는 여기를 제자리에서 패치하지 않는다 — 새 엔진 관측은 가드너 `exports/` 패치 제안 → 사용자 승인 → 정본 릴리스 경로로만 들어온다. 실존 인물 관측 기하와 운영자 취향 기본값은 공개 배포되는 이 레포가 아니라 로컬 전용 `higgsfield-prompt-bridge`가 보유한다.
