...
운영 소유권(2026-08-02): Codex가 `~/HeiTuz/MPW`를 정본 편집 표면으로 사용한다. Codex는 이 트리에서 규칙을 편집하고 로컬 커밋할 수 있지만 원격 push는 별도 승인 없이는 하지 않는다. Claude Code와 Hermes는 `~/.claude/skills/MPW`·`~/.hermes/skills/prompt-writing/MPW`의 소비자이며 설치 경로에서 직접 편집·커밋하지 않는다. Codex 설치본(`~/.codex/skills/MPW`)은 `agents/codex/` 오버레이가 적용된 복사 소비 산출물이다. 정본 변경 뒤 설치본을 재생성하고 doctrine 패리티 검사를 통과시킨다. 버전 범프와 릴리스 push는 Codex가 정본 커밋·검증을 끝낸 뒤 별도 릴리스 승인으로만 수행한다.
