...
| 4 | HEX 팔레트 | 사용자 지정 색·원본 색 보존을 우선한다. 자유롭게 제안할 색에서만 사조 HEX를 프리셋 HEX보다 우선하며, 전달 방식은 [surfaces.md](surfaces.md) §4.1을 따른다. |
...
| 제한 | 비중을 지정했다면 합은 100. 최종 색은 요청한 팔레트 또는 선택한 전문 레인의 계약을 따르며, 색 개수를 채우기 위해 확장하지 않는다. 파라미터 우선은 [surfaces.md](surfaces.md) §4를 따른다. |
