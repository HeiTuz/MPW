# AGENTS.md — MPW 플러그인 payload (ChatGPT·Codex·Claude Code)

이 디렉터리는 ChatGPT·Codex·Claude Code 플러그인에 번들된 MPW **스킬 payload**다. 정본 저장소가 아니다 — 저장소 기여 규칙(커밋·리뷰·정본 단일성 유지보수)은 여기에 적용되지 않는다.
...
- 이 트리를 제자리에서 편집하지 않는다. 수정은 정본 https://github.com/HeiTuz/MPW 에서 하고 `node scripts/build_plugin.mjs`로 플러그인을 다시 생성한 뒤 마켓플레이스에서 재설치한다.
