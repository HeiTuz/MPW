---
name: MPW
description: "프롬프트 작성·퇴고·라우팅 전용 통합 스킬 — '프롬프트 만들어줘/다듬어줘/검토해줘' 류 요청에 발동. 소관: 자율 goal 루프 지시문, 팀·멀티에이전트 작업지시 프롬프트, 시스템 프롬프트·자동화 잡, 직무 업무 프롬프트, 모델 튜닝·퇴고, UI/디자인 프롬프트 정밀 컴파일. 문장을 쓰기 전에 의존 순서·축별 권한자·배제 관계를 그래프로 세운 뒤 조립한다(references/prompt-graph.md). 길이·비율·해상도는 보편 상수가 아니라 실행 표면의 속성으로 다룬다 — 전달 채널 / 타깃 엔진 / 기계 계약 중 가장 좁은 상한이 구속하며(전역 2000자 하드라인은 없고 미드저니는 문자가 아니라 단어 수로 잰다) references/image/surfaces.md에서 먼저 판정하고, 이미지·영상 모델 선택은 references/image/model-routing.md의 목적축 표를 따르고, 선택한 엔진의 붙여넣기 문법·시트 변환·피드 진단(Grok Imagine, Seedream 5 Pro·캐릭터 시트, Seedance 2.0 direct, Dreamina Seedance 2.5, Midjourney 피드 진단)은 같은 파일 §4 규칙 7의 표면 어댑터가 소관이다. 실행자가 파라미터를 갖는 축은 산문에 중복 기술하지 않는다. 이미지·디자인 레퍼런스 분석 결과를 생성 프롬프트로 컴파일하며, 런타임별 호출 문법은 references/adapters.md를 따른다. 프롬프트 산출 없이 실제 코드 실행·이미지 생성·문서 조판만 원하는 요청은 이 스킬이 아니라 해당 실행 경로 소관."
version: 2.28.0
license: MIT
metadata:
  ...
  host_surface: claude
  canonical_source: "HeiTuz/MPW SKILL.md v2.28.0"
  updated_at: "2026-08-03"
  model_claims_reviewed_at: "2026-08-03"
...
> **호스트 통합 — Claude Code.** 이 파일은 Claude Code 설치본(`~/.claude/skills/MPW`)의 진입 표면이다. 규칙 본문은 정본 SKILL.md와 동일하며, 호스트 통합 표면(프런트매터·발동·도구 명칭)만 마이그레이션됐다.
...
**금지되는 것은 눌러도 아무 일도 없는 장식 메뉴다.** 아래 후속 안내는 MPW에 실제로 있는 연산을 가리키므로 장식이 아니다. 블록 뒤에 번호 붙인 선택지로 이어붙인다.
...
**자동 개선은 축을 바꾸지 않는다.** 무엇을 요청했는지는 그대로 두고 **얼마나 잘 표현됐는지**만 고치는 품질 패스다: 모호어를 [templates.md](references/templates.md) 치환표로 바꾸고, 단독으로 놓인 하중 토큰에 화면 결과를 병기하고, 저신호 어휘를 결과로 환원하고, 중복·장식을 걷어내고, 얕은 슬롯을 관측·기본값으로 채운다. 판정 근거는 이 파일의 **출력 게이트**이고, 거기 걸리는 것만 고친다.

축을 바꾸는 것(소품·조명·컬러…)은 **MODEL-델타**이고 2번 이후의 일이다. 둘을 같은 연산으로 묶으면 사용자가 "더 좋게"를 눌렀는데 다른 그림이 나온다.

**1회만 돈다.** 반복 다듬기는 자가수렴 오버런이다. 게이트에 걸리는 게 없으면 고치지 말고 "개선할 지점 없음"이라고 말한 뒤 원본을 유지한다 — 멀쩡한 프롬프트를 흔드는 것이 가장 흔한 실패다.

**2번부터는 이 산출물에서 실제로 값어치 있는 것만 에이전트가 판단해 1~3개 붙인다.** 고정 목록을 돌려쓰지 않는다 — 매번 같은 세 줄이 나오면 그게 곧 장식 메뉴다.

후보는 MPW에 실제로 있는 연산이어야 한다: 축 지정 **MODEL-델타**(1번과 달리 결과가 달라진다) / 표면 재판정 후 **다른 모델·플랫폼으로 재컴파일**([surfaces.md](references/image/surfaces.md) → [model-routing.md](references/image/model-routing.md)) / 컷 분리 / 레인 전환 / 그 표면의 파라미터 조정 / 체인 다음 단계로 넘기기 / IMAGE 산출물의 실행 핸드오프(아래 규칙).

**IMAGE 산출물에는 실행 옵션이 마지막 번호로 붙는다.** 산출물이 로컬 이미지 러너(ImgGen2)가 소비할 수 있는 형태이고 같은 호스트에 러너가 설치돼 있으면, 목록의 마지막 번호는 `imggen2로 바로 생성 — 이 프롬프트(배치) 그대로 실행`으로 고정한다. 조건 판정·호출 배선은 [references/adapters.md](references/adapters.md) §이미지 생성 실행 옵션이 정본이다. 붙여넣기 전용 엔진·생성 MCP 런타임처럼 전용 실행 경로가 있는 표면과 러너가 없는 호스트에는 붙이지 않는다.

**판정 한 줄: 그 항목을 골랐을 때 지금 이 산출물에 대해 다른 결과가 나오는가.** 아니면 뺀다. 일반론(`더 자세히`, `다른 스타일로`)은 연산이 아니다.

채워진 예 — 미드저니 프롬프트 10개를 낸 직후라면:

```text
다음:
1. 자동 개선 — 의도는 그대로 두고 디테일·표현만 강화
2. 무드보드 ID를 물려 재컴파일 (--p, 강도는 --s로만)
3. 소품 축만 다시 굴리기
4. 같은 감도로 룩북 컷 별도 컴파일 (샷 사이즈가 달라 별도 레인)
```

**이 턴이 실제로 프롬프트 블록을 냈을 때만 붙인다.** 스킬 자체를 고치는 턴, 검증·배포 보고, 설치 상태 확인, 질문에 답만 한 턴에는 붙이지 않는다 — 컴파일된 산출물이 없으면 "다음"이 가리킬 대상도 없다. 이모지 번호·"선택하세요"·표는 쓰지 않는다. **턴을 멈추는 대화형 선택 도구로 내지 않는다** — 버튼은 고르기는 편하지만 그냥 받고 끝낼 자유를 뺏고, 대기 상태로 턴이 끝난다. 평문 번호는 골라도 되고 무시해도 되며 런타임을 가리지 않는다. 사용자가 이미 다음 행동을 말한 맥락이면 붙이지 않고, 자율 루프·기계 핸드오프(S1)에서도 붙이지 않는다 — 읽을 사람이 없다.
