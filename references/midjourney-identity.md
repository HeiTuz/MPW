...
- 누출이 남으면 순서대로: ① 슬롯을 `--sref`로 옮긴다 ② `--sw`를 낮춘다 ③ 텍스트에서 피사체와 의상을 과잉 특정해 레퍼런스 의상이 들어올 자리를 없앤다 ④ 레퍼런스를 클로즈업에서 환경 샷으로 바꾼다(포트레이트가 더 많이 샌다). 구체적인 `--sw` 조절 수치표는 단일 커뮤니티 출처뿐이다 **[미확인]**.
...
**문자 수 게이트를 이 엔진에 적용하지 않는다.** 붙여넣기 배선에서 유효 상한이 블록당 2000자로 떨어지는 것은 [image/surfaces.md](image/surfaces.md) §0-1의 채널·계약 층 소관이고, Midjourney 프롬프트는 그 한참 아래에서 끝나므로 실효 제약이 아니다.
