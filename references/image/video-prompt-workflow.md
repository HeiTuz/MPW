...
- 다중 샷 광고·몽타주의 리듬 설계 → §다중 샷의 강약을 검토하는 예
...
Genjutsu의 원본 변환은 [higgsfield-genjutsu.md](higgsfield-genjutsu.md)로 분기한다. 아래 카메라만 가져오는 예나 새 시간별 사건 구성은 해당 기능의 지원 확인 없이 적용하지 않는다.
...
브리프가 “작은 금속 공구의 정밀함을 보여주는 짧은 광고, 컷 전환 가능”이라면 먼저 관객에게 보여줄 정보를 샷별로 나눈다. 예를 들어 첫 샷에서 공구 끝이 부품에 닿는 순간, 다음 샷에서 맞물린 단면, 마지막 샷에서 완성된 조립 상태를 보여줄 수 있다. 빠른 접촉 뒤 단면을 읽는 정지 구간을 두고, 전환은 공구의 운동 방향이나 형상이 이어지는 지점에 건다. 이때 속도 변화·입자·블러를 모두 한 샷에 넣어 접점을 가리지 않는지 확인한다.

이런 샷별 정보·전환·강약 점검은 **편집이 중요한 요청에서만** 작성 중에 쓴다. 효과 인벤토리나 밀도표는 샷이 많아 반복을 놓칠 때의 검토 메모이며, 최종 생성 프롬프트의 필수 섹션이 아니다. 원테이크·조용한 관찰·사용자가 고정한 컷에서는 새로운 효과나 전환을 만들지 않는다. 실제 컷 수와 길이는 선택한 표면의 지원 범위와 사용자가 요구한 편집 구조에 맞춘다.
...
### XAZINGA 스킬 검토 (2026-09-23)

[Seedance 2.0 프롬프팅](https://www.xazinga.com/skills/57eca570-2d6c-4d4b-84eb-e50a9f124b29), [Seedance 2.5 프롬프팅](https://www.xazinga.com/skills/a18ef1b5-42b0-4f97-8b4d-6f360535dca4), [비디오 프롬프트 빌더](https://www.xazinga.com/skills/5b54b7f3-9a48-4e58-8828-738a97fb7af9), [Seedance 프롬프팅 가이드](https://www.xazinga.com/skills/e33fa879-481a-4506-96c1-cfbc6713ff6a), [Seedance 디렉터](https://www.xazinga.com/skills/e87a26be-1d6d-48d3-98de-f79d01dd4496)의 공개 본문과 제공된 2.0·2.5 zip 내용을 검토했다. 자산별 역할, 장면의 끝 상태, 편집·연장 경계는 MPW의 [seedance-2.md](seedance-2.md)·[seedance-2-5.md](seedance-2-5.md)에 이미 반영돼 있었고, 장면 목적에 따른 카메라 선택도 [lanes.md](lanes.md) §연출을 고르는 기준에 있었다. 추가로 다중 샷의 효과 반복·강약 점검을 이 문서와 해당 기준에 선택지로 반영했다.

사이트가 제시한 고정 3초 분할·샷 수·4/5섹션 의무, 영어+중국어 이중 출력, 렌즈·속도 수치의 성능 우위, 장면 유형별 금지 규칙은 통제된 생성 비교나 공식 표면 계약으로 확인되지 않았다. 이를 보편 규칙이나 엔진 사양으로 채택하지 않는다. 이 검토는 공개된 작성법의 비교이며 실제 영상 생성 품질을 검증한 결과가 아니다.
