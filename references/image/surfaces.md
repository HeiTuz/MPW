...
- UI에서 고르는 값(모델·비율·프리셋·품질·길이)은 프롬프트 본문이 아니라 **별도 라벨 한 줄**로 전달한다. GPT Image의 미지정 모델은 [model-routing.md](model-routing.md) §4의 분기로 정한다. 실제 설정이 없는 대화형 도구에서 자연어 요청을 지원하는 축은 본문에 남긴다(예: 아래 §4.2 Grok 대화형 이미지 도구).
...
새 생성은 용도·주체·구도·눈에 보이는 스타일·필수 조건을 짧은 완결본으로 쓴다. 짧은 문장·서술 문단·JSON형 구조·지시문·태그는 모두 같은 의도를 전달할 수 있으므로 읽고 고치기 쉬운 형식을 고르고, 특수 문법이 결과를 보장한다고 보지 않는다. 복잡한 요청만 장면·주체·세부·제약으로 구획을 나누며, 2.5라는 이유로 고정 템플릿이나 긴 태그 묶음을 붙이지 않는다. 아래는 공식 프롬프팅 가이드(2026-09-16 재확인)를 반영한 작성 기준이며 생성 품질 실측 결과는 아니다(근거는 §7).

- **눈에 보이는 세부:** 재질·빛·색·매체를 이름으로 적고, 실사가 목표면 `photorealistic`·`real photograph`를 명시한다. 카메라·렌즈 사양은 외형 단서이지 물리 재현 보증이 아니다. 광각·시네마틱·저조도·비·네온 장면은 분위기 단어 대신 규모·대기·색을 적는다. 인물은 신체 프레이밍·상대 크기·시선·사물과의 상호작용을 지정한다(예: `full body visible, feet included`).
...
- **정확한 글자:** 카피를 따옴표로 묶고 위치·위계·출현 횟수를 지정한다. 낯선 단어·브랜드명은 필요하면 낱자로 철자를 적고, 추가 문구 금지를 함께 쓴다. 한글 철자·작은 라벨·불필요한 추가 문구를 결과에서 확인한다. 작은 글자·밀집 정보·여러 서체가 있으면 §4.3의 품질 단계를 한 단계씩 비교한다. 도해·역사 장면은 사실관계도 별도로 확인한다.
...
**편집 패턴 — 공식 예시의 지시 핵심.** 마스크가 필요한 국소 편집은 §5의 마스크 조건을 따른다.
...
| 슬라이드·차트·다이어그램 | 산출물 사양처럼 쓴다 — 정확한 산출물명·캔버스·위계·실제 텍스트와 수치·시각 언어·가독성 제약. 덱형은 가로 크기, 작은 글자·범례·축·각주는 §4.3의 품질 단계 비교 |
