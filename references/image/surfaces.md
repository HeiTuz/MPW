...
JSON 형식 자체가 S1을 뜻하지 않는다. 플랫폼 호출 인자를 JSON으로 쓰는 요청은 S2다. 미지정이면 특정 UI를 가정하지 않고 문법 중립적인 자연어로 쓴다. 이 경우 모델·표면별 검사는 아직 적용하지 않으며, 지원 필드·미디어 바인딩·정확 길이 등 대상 없이는 완성할 수 없는 요청에만 선택을 확인한다. 표면이 바뀌면 필드·길이·참조 전달을 다시 검증한다.
