...
| 2 | L1~L9 룩 프리셋 | 요청했거나 룩 구분이 결과를 바꿀 때만 마감 톤 1개를 선택한다. 프리셋 혼합은 팔레트 충돌로 금지한다. |
| 3 | M/R/X/T 축 | 컨셉 다변화가 필요할 때 축 1개만 선택한다. |
| 4 | HEX 팔레트 | 사용자 지정 색·원본 색 보존을 우선한다. 자유롭게 제안할 색에서만 사조 HEX를 프리셋 HEX보다 우선하며, 전달 방식은 [surface-contracts.md](surface-contracts.md) §4.1을 따른다. |
...
이 표는 **의미가 불명확할 때만** 쓴다. `잘/예쁘게/고급스럽게/세련되게/감도있게`, `어워드 수준으로/전문가처럼/최고급`이 주어진 자료로 충분히 특정되면 유지할 수 있다([compiler.md](compiler.md) 철칙 3). 불명확할 때만 아래 경로로 환원하며, 사용자가 주지 않은 수치를 새로 만들지 않는다(철칙 5). 검증기 `W-FILLER`는 확인용 경고다.

| 경로 | 방법 | 예 |
|---|---|---|
| 수치 | 사용자·브랜드 가이드가 준 여백 비율·색 비중·위계 단수가 있을 때만 그 값으로 | `여백 60%, 메인 #F5F1E8 60 / 보조 #1C1A17 30 / 포인트 #8C7B6B 10, 위계 3단` |
...
| 2 | 그 장면을 필요한 HEX 팔레트로 번역한다. 주조·보조·강조의 역할과 색의 유래를 정하고, 면적 배분이 중요한 요청에만 비율을 정한다. |
| 3 | 번역된 팔레트를 Color grading 섹션에 넣는다. 장면 문장은 프롬프트에 넣지 않는다. |
| 제한 | 비중을 지정했다면 합은 100. 최종 색은 요청한 팔레트 또는 선택한 전문 레인의 계약을 따르며, 색 개수를 채우기 위해 확장하지 않는다. 파라미터 우선은 [surface-contracts.md](surface-contracts.md) §4를 따른다. |
...
렌더 텍스트를 다루므로 Tier-1 결합 공식 1회가 필수다. 한글 카피는 표면별 캔버스 크기 설정([surfaces.md](surfaces.md): S1 기계 계약→스키마 enum, S1-legacy→production.md §11 사이즈락 6종 긴 변 2048, S2→모델 정의, S3→UI 라벨)을 적용한다. 영역 문법은 `typography.md`를 참조한다.
