...
| 4 | 사용자 지정 혼합 문구는 그대로 보존. 새 카피 설계일 때만 줄·롤 분리를 고려. | `W-TEXT-MIXLANG`는 확인용. 정본 compiler.md §6. |
...
| 텍스트량 | 품질 축 | 해상도 축 |
|---|---|---|
| 텍스트 블록 3개 이상 또는 작은 활자(캡션·라벨·본문) | 선택 표면이 제공하는 최상단 | 선택 표면이 제공하는 최상단 |
| 짧은 카피 1~2줄(타이틀+서브) | 기본값으로 판독 가능하면 유지 | 표면 기본값으로 판독 가능하면 유지 |
| 카드뉴스 본문·인포그래픽·도감 라벨(밀집) | 선택 표면이 제공하는 최상단 필수 | 선택 표면이 제공하는 최상단 필수 |
...
| 3 | 카피 길이가 레이아웃을 밀어냄 | 레이아웃 조정 | 지정 카피는 그대로 두고 위치·줄바꿈·위계를 조정한다. 작성자가 만든 조절 가능 카피만 축소할 수 있다. | 지정 정보가 여전히 들어가지 않으면 4로 이동 |
| 4 | 정보량이 현재 구성에 맞지 않음 | 모델·구성 재검토 | 지원되는 부분 편집·다른 모델·레이아웃을 검토한다. 출력 수가 허용할 때만 컷을 분할하며, 한 장으로 잠겼으면 자동 분할하지 않는다. | 카피와 출력 계약을 함께 충족하지 못하면 충돌을 보고 |
...
| KO/EN 포스터 | 지정 혼합 문구 보존 + 필요한 롤 라벨·Tier-1 + §7 표면별 레버 |
| 도감 라벨 | 필요한 callout/caption + §7 표면별 레버 |
| 카드뉴스 본문 | 요청 정보의 읽기 순서 + §7 표면별 레버, 출력 수 보존 |
| 타이포 아트 | 요청한 T/TP 구조 + 해당 텍스트 계약 + §7 표면별 레버 |
