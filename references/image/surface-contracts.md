...
- **길이: 이 표면 자체는 상한을 계약으로 갖지 않는다. 그렇다고 무제한이라는 뜻은 아니다** — §0-1의 나머지 두 층이 그대로 살아 있다. 타깃 모델의 상한(Higgsfield 모델은 미공개 **[미확인]** — 근거·확인일은 [surface-evidence.md](surface-evidence.md) §7)과 전달 채널 상한 중 좁은 쪽이 구속하고, 둘 다 넉넉하면 그때 신호 밀도로 관리한다. "S2니까 길이 제한 없음"으로 끝내지 않는다.
...
- **정확한 글자:** 카피를 따옴표로 묶고 위치·위계·출현 횟수를 지정한다. 낯선 라틴 문자 단어·브랜드명은 필요하면 낱자로 철자를 적고(한글은 하이픈이 글자로 렌더되는 실측 사고가 있어 낱자 풀어쓰기를 하지 않는다 — [typography.md](typography.md) §4 정확 문자열 프로토콜 3행), 추가 문구 금지를 함께 쓴다. 한글 철자·작은 라벨·불필요한 추가 문구를 결과에서 확인한다. 작은 글자·밀집 정보·여러 서체의 품질 단계 비교는 §4.3 설정을 실제로 제공하는 표면에서만 적용한다. 도해·역사 장면은 사실관계도 별도로 확인한다.
...
| 영상 길이 | `duration` (모델별 **열거값 또는 범위**) | 쓰지 않는다. 임의 초 지정 금지. 엔진 공식 형식이 정렬 기준으로 같은 값을 요구하면 파라미터와 일치시켜 쓴다(예: [minimax-h3.md](minimax-h3.md) 프레임 모드 `S.SS`) |
...
필드가 없다는 것은 **자연어 부정문을 이해하지 못한다는 뜻이 아니다.** GPT Image 공식 가이드는 바꿀 부분과 보존할 부분, 추가 금지를 직접 명시한다. MPW의 `compiled` 프로필과 명시된 Tier-0/1/2 계약은 [compiler.md](compiler.md) §2대로 유지하고, 이를 모든 네이티브 모델의 문법으로 일반화하지 않는다. 근거·확인일은 [surface-evidence.md](surface-evidence.md) §7에 있다.
...
- **본문 HEX:** 색마다 적용 대상과 역할을 연결한다. 예: `배경은 따뜻한 크림 #F5F1E8, 제목은 버건디 #722F37`. 코드만 나열하거나 화면 전체의 컬러 그레이드와 특정 물체의 고유색을 혼동하지 않는다. FLUX.2의 본문 HEX 지원은 [surface-evidence.md](surface-evidence.md) §7의 공식 가이드로 확인되지만, 다른 모델의 반영 정도는 해당 표면의 문서·관측으로 판정한다.
...
Grok 전용 공통 prompt 문자 상한·최적 단어수는 확인한 가이드에 제시되지 않았다 **[미확인]**. §0-1의 실제 채널·기계 계약 상한을 적용하고, 알려지지 않은 숫자를 강제하지 않는다. 자연어 작성은 [grok-imagine.md](grok-imagine.md), 근거는 [surface-evidence.md](surface-evidence.md) §7을 따른다.
