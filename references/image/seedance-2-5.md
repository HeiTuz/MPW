...
아래 순서는 내부 조립용이다. 예시의 헤딩·Use/Event/End state 라벨은 필수 UI 문법이 아니며, 필요한 관계·사건·끝 상태를 보존하면서 사용자 지정 형식에 맞춘다.

1. **모드 판정** — 신규 생성 / 30초 생성 / Long Video / 편집 / 순방향 연장 / 역방향 연장 / 키프레임 / 스토리보드 / 클레이 렌더러 중 해당하는 모드를 내부에서 먼저 정한다. UI에서 선택한 모드명은 본문에 중복하지 않는다.
...
클립별 자산 준비와 업로드 순서표는 [video-prompt-workflow.md](video-prompt-workflow.md) §참조 준비와 클립별 입력 순서를 따른다. 공개 제작 기록을 가져올 때도 이 어댑터의 실제 표면 계약으로 변환하며, 기록에 남은 토큰을 그대로 붙여넣는 것만으로 자산 연결이 완료되지는 않는다.
...
멀티샷 시퀀스에서는 샷마다 카메라 위치·렌즈 느낌·움직임을 다르게 지정할 수 있다(관측 2026-09, 출처 1건). 한 컷의 끝 상태와 다음 컷의 시작을 같은 동작·형상으로 잇는 매치컷은 연결 지점을 명시한다. 캐릭터·의상·배경을 디자인 시트로 참조할 때는 시트의 흰 배경·라벨·레이아웃을 가져오지 않는다고 배제 축에 쓴다.
