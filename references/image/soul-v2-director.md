...
1. **Optimized Soul V2 Prompt** — 복사 가능한 코드블록 1개. 한 문단, 4레이어 순서, 내부 헤딩·설명·대안 없음, 붙여넣기 즉시 사용 가능. **길이 상한은 표면이 정한다**([surfaces.md](surfaces.md)): 상한 있는 채널 배선으로 나갈 때만 그 상한이 하드라인이고 `길이:` 노트에 실측값을 적는다. 그 외에는 [surfaces.md](surfaces.md) §0-1의 세 층 판정(전달 채널/타깃 엔진/기계 계약 중 가장 좁은 것)을 따른다. 플랫폼 파라미터 표면(S2 — Higgsfield MCP 등 모델 id로 호출)에는 길이 상한 계약이 없으므로 문자수를 맞추려 신호를 버리지 말고 밀도로 관리한다.
