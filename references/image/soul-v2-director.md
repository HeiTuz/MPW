...
- 색을 연결하려면 지정 팔레트의 색을 의상과 주변 사물에 자연스럽게 반복한다. 색의 역할과 대상이 읽혀야 하며 반복 횟수·광택 소품은 필수가 아니다. HEX 입력과 Color Transfer의 지원 경계는 [surface-contracts.md](surface-contracts.md) §4.1을 따른다.
...
- 비율·품질·Color signature는 본문과 분리한다 ([lanes.md](lanes.md) §인물·사실감 이미지 레인, [surface-contracts.md](surface-contracts.md) §4). S1은 계약 필드, S2는 실제 호출 파라미터, S3는 UI 라벨 줄이 권한자다. 기본값은 설치가 공급하고, 없으면 [lanes.md](lanes.md) §이미지 슬롯 기본값 절차로 고른다.
...
목적에 맞는 시스템 하나만 고른다. 미디엄 포맷 스튜디오 카메라로 raw 디스포저블 컨셉을 찍지 않는다. 아래 표의 필름 상표·조명 장비명은 **선택용 참고**다 — Soul 최종 프롬프트에는 §실측 법칙 3에 따라 상표명을 빼고 발색·그레인으로, 장비명(softbox·octabox·reflector)을 빼고 빛의 방향·넓이·그림자 성질로 옮겨 쓴다.
...
- 필름 스톡 언어는 색 반응·그레인 제어이지 물리 필름 프레임 허가가 아니다. 보더리스는 상표명 없이 발색으로 쓴다(예: `warm natural skin tones, fine grain, softly lifted blacks`). 상표명을 넣으면 필름 테두리·각인이 그려진 실측이 있다(§실측 법칙 3).
...
**색상 보정(2026-09-06 공식 문서 확인):** 이전 메모의 HEX 실패 주장은 본문 코드 입력과 Soul HEX 기능을 구분하지 않았고, 이 절에 색상별 대조 결과도 남아 있지 않아 보편 규칙으로 유지하지 않는다. 색 제어는 [surface-contracts.md](surface-contracts.md) §4.1, 공식 근거는 [surface-evidence.md](surface-evidence.md) §7을 따른다. 본문 HEX의 현재 반영률은 별도 생성 대조 전까지 미확인이다.
