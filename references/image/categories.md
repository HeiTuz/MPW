...
| 텍스트 렌더 가드 | 렌더 텍스트가 있을 때만 [compiler.md](compiler.md) §2의 Tier-1 가드를 적용한다. |
...
| **B. 컷 단위 생성 후 조판** | 정밀 통제가 우선 | 1컷=1호출. persona 블록 반복. 요청한 컷의 주체 수와 역할을 유지하며, 단독 인물 컷일 때만 "프레임 안엔 인물 한 명, 단독 포트레이트"를 긍정 단언한다. |
...
| 1행 원칙 | 1행 = 1슬라이드 = 1호출. 덱은 jsonl 챕터로 관리한다. 렌더 텍스트가 있으면 Tier-1 승격 후보(텍스트 블록 3개+ 조건)로 보고, 텍스트가 없으면 Tier-0으로 둔다. |
| 룩 | 요청했거나 룩 구분이 결과를 바꿀 때만 룩 프리셋 1개를 골라 덱 DNA 블록에 인라인한다. |
