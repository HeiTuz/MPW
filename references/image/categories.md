...
| 모드 | 사용 조건 | 작성법 |
|---|---|---|
| **포스터형 `poster_dense` 기본** | 기본값 | 섹션 밴드 4~6개 + 섹션마다 번호·헤더. 셀마다 아이콘/미니 일러스트. **히어로 요소 1개**(실사급 렌더 — 김 나는 컵, 단면 등)를 상단 또는 중앙에 둔다. 장식 막대·게이지·미니 지도·비교표를 보조로 깔아 밀도를 채운다. 배경은 종이·크라프트·빈티지 등 질감 있는 톤. **품질 축과 해상도 축을 그 표면에서 쓸 수 있는 최상단으로 올린다** — 한글 정확도가 캔버스 픽셀 높이에 비례한다는 실측이 근거이고, 특정 픽셀값 지시가 아니다. 표면 판정과 값 소관의 정본은 [surfaces.md](surfaces.md): S1 기계 계약이면 스키마 `size`·`quality` enum 안에서 최상단, S1-legacy 벌크면 사이즈 락 6종 안에서 긴 변 2048(정사각 밀집)·1536(세로 비율 우선)이고 6종 밖은 금지, S2면 `resolution` 2k 이상 + `quality: high`, S3면 UI 라벨 줄로 고지한다. |
