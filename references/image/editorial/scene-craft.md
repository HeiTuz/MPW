# 에디토리얼 패션 — 장면·포즈·소재·조명

우선순위: [../lanes.md](../lanes.md) §레인 게이트 카드 > [../compiler.md](../compiler.md) 철칙 > 이 파일. 철칙 전문은 [../compiler.md](../compiler.md).

## 8. 포즈·시선 디렉션 어휘

**방향 기준 규칙 (2026-07 실측).** 시선·조명 방향이 결과를 가를 때만 명시하고, 방향을 쓸 때는 뷰어 기준 영어 토큰(`camera left`/`camera right`)을 사용한다 — 한국어 "왼쪽/오른쪽" 단독은 피사체 기준과 뷰어 기준이 갈려 실측에서 방향이 뒤집혔다.
...
계절·장소는 의상과 빛의 이유를 만든다. 사용자 지정 인물·브랜드·행사는 보존하며, 새 컬렉션 설정이 필요하지만 지정되지 않았을 때만 가상으로 작성한다.
