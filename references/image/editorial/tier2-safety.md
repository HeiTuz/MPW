...
- 의상 슬롯의 4요소(색·소재·의류종·핏/디테일)는 사용자 지정·참조 관찰·위임된 창작 범위 안에서 구체화한다. 부분 수정이나 원본 보존 작업에서는 빈 요소를 채우려고 의상을 새로 설계하지 않는다. `불투명(opaque)`은 노출성 의상 어휘가 하나라도 있으면 소재 수식어로 상시 부착한다.
