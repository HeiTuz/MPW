...
판정문: **단독 인물 화보는 Format B 우선**이다. V4 10섹션은 룩북/챕터 시퀀스용이다. (레거시 주의: V4의 메타 필드·8룩 폴더 규칙은 별도 룩북 프로젝트 체계에서 온 것이다. 이 레포의 러너 레코드 스키마([../production.md](../production.md) §2)에는 없는 필드이며, 예제·fixtures·검증기는 전부 Format A/B 체계다 — V4 메타를 jsonl 필드로 승격하지 않는다.)
