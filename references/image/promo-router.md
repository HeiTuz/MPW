...
6. `promo_text_effect`가 `mask` 또는 `extrusion`이면 한글 안전권은 2음절까지다. 3음절 이상은 이 효과로 방출하지 않는다. 지정 카피는 보존하고 허용된 효과 변경을 검토한다. 축소는 작성자가 만든 조절 가능 카피에만 적용하며, 카피와 효과가 모두 고정이면 충돌을 보고한다.
