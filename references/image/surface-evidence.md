...
| ChatGPT 웹 이미지 생성·편집 | OpenAI [Images in ChatGPT](https://help.openai.com/en/articles/11084440): Images 2.5, 첨부 이미지 편집, 선택 영역 또는 자연어 위치 지정, 비율 선택 또는 본문 지정, 투명 배경 요청. 영역 밖 변경 가능. API 모델 ID·quality 선택이나 개별 계정의 실행 모델 확인은 아님 | 2026-09-23 |
...
| Higgsfield `minimax_h3`·`minimax_h3_max` 로스터 | `models_get` 직접 조회 — `minimax_h3`: duration 4–15초(default 5)·resolution `2K` 단일·batch 1–4·`aspect_ratios` 7종, `minimax_h3_max`: duration 5–15초(default 5)·resolution 480p/768p·batch 1–4. `medias.roles` = start_image/end_image/image_references/video_references/audio_references. `negative_prompt` 계열 없음. `minimax_h3_max`의 이미지·영상·오디오 참조 조합 모두 확인됨(아래 프로브 행) | 2026-09-18 |
| MiniMax H3 Max 참조 실측(Higgsfield 경유) | `generate_video` `minimax_h3_max`에 이미지 참조 2장(`image`→`image_references` 자동 매핑)으로 5초·16:9 생성 성공 — 결과 영상이 프롬프트의 `<Picture 1>`/`<Picture 2>` 배치대로 재현. 이어서 참조 영상 1개(`video_references`)·참조 오디오 1개(`audio_references`) 조합으로도 5초 생성 성공(참조 레이아웃 재현, 1344×768·24fps·5.17초·AAC). 생성 품질 일반화 아님(조합별 1회) | 2026-09-18 |
