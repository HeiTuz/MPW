...
- `detailed_description`: 샷별 상세 본문. 생성 과업의 공식 권장 분량은 영어 350–500단어지만 최소 분량 계약은 아니다. 필요한 상세와 사용자의 명시 길이 조건을 우선하며 분량을 채우려고 새 사건을 추가하지 않는다. 대사가 많으면 발화 타임라인 수용을 우선한다. 기본 모드와 달리 스타일은 `[Shot 1]` 앞에 한두 문장으로 먼저 세운다.
- 끝의 두 섹션은 기본 모드와 같은 정의다.

**서술은 영어를 기본으로 쓴다.** 명시 언어·원문 보존과 실제 도구의 언어 제약이 충돌할 때는 [common.md](../templates/common.md) §프롬프트 언어 결정의 확인 경로를 따른다. 대사·가사와 화면에 실제 보이는 텍스트만 원어를 유지한다 — 대사·가사는 `<d>` 태그 안에, 화면 표기 텍스트는 큰따옴표 안에 원문 그대로 둔다.
