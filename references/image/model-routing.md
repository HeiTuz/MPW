...
**0. 미지정 기본값은 표면부터 구분한다.** 이미지 요청에 타깃이 없으면 설치의 기본 이미지 경로를 유지한다. OpenAI 직접 API에서 새 모델을 고를 때는 아래 2.5 기준을 적용한다. Higgsfield의 기본값은 기존 `gpt_image_2`이며, 공개 2.5 출시만으로 래퍼 id를 바꾸지 않는다. 모델 선택을 노출하지 않는 대화형 이미지 도구는 호스트 선택을 따른다. §1 목적축 표는 Higgsfield 후보를 좁히는 도구이며 다른 표면에 이식하지 않는다.

**GPT Image 2.5 선택 (2026-09-09 공식 문서 확인):** 속도가 우선이거나 기존 GPT Image 2 품질이 충분하면 `gpt-image-2.5-flare`부터 비교한다. 복잡한 편집·브랜드 디테일 등에서 기존 품질이 부족하면 `gpt-image-2.5-sunburst`부터 비교한다. 사용자 지정 모델은 유지한다. 첫 비교는 프롬프트·참조·크기·공통 품질 설정을 고정하고, 품질을 충족한 뒤 속도를 비교한다. 빠른 모델이 더 싸거나 모든 컷에서 일정 비율로 빨라진다고 단정하지 않는다. 자연어 작성은 [surfaces.md](surfaces.md) §3.2, API 설정은 §4.3을 따른다. 공식 근거·확인일은 같은 파일 §7에 있다.
