...
FILES = ["SKILL.md", "references/image/from-image.md", "references/image/ghost-character-reference-sheet.md", "references/templates.md", "references/model-playbooks.md", "references/adapters.md",
...
def check_plaintext_paths(root, errors):
    ...
    repository_files = [path for path in root.rglob("*") if path.is_file() and path.relative_to(root).parts[0] != "build"]
    unresolved = set()
    for source in files:
